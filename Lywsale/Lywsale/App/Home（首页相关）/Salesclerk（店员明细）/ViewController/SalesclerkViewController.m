...
- (void)viewDidLoad {
    ...
    self.salesclerkView.dataSources = @[@"", @"", @""];
}
...
- (UIView *)headerView {
    ...
    if (!headerView) {
        ...
        NSArray *titles = @[@"销售额", @"订单数", @"客单价", @"新增会员"];
...
- (SalesclerkView *)salesclerkView {
    
    if (!salesclerkView) {
        
        salesclerkView = [[SalesclerkView alloc] initWithFrame:CGRectMake(15, self.headerView.height, ScreenWidth - 30, ScreenHeight - self.navHeight - self.headerView.height) style:UITableViewStyleGrouped];
        [self.view addSubview:salesclerkView];
    }
    
    return salesclerkView;
}
